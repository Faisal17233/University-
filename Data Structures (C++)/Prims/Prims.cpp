...
void Prims(Vertex* start) {
	...
	Arr* arr = new Arr[VertexCount];// for saving the connected vertexes
	int n = 0;//vertex saved Arr
	arr[0].vertex = start;
	...
	while (true) {
		...
		if(check == 1){
			...
			if (LoopDetect(save->source->vertex) == 1) { // Checking and deleting if the inserted edge made a loop
				...
				DeleteEdge(save->source->vertex, save->desti->vertex);//deleting from graph also so it doesnt get connected again
